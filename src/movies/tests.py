...
class CommentViewTest(TestCase):
    ...
    def test_get_comments(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.data['results'][0]['comment'],
            comment_dict['comment'])

    def test_post_comment(self):
        response = self.client.get(
            reverse('comments'), {
                'comment': 'test', 'movie': self.id})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
...
class TopMoviesTest(TestCase):
    ...
    def setUp(self):
        ...
        self.expected_response = [{'movie_id': 102,
                                   'total_comments': 5,
                                   'rank': 1},
                                  {'movie_id': 101,
                                   'total_comments': 2,
                                  ...
                                  {'movie_id': 100,
                                   'total_comments': 2,
                                   'rank': 2}]
        self.expected_response_short = [
            {
                'movie_id': 101, 'total_comments': 2, 'rank': 1}, {
                'movie_id': 100, 'total_comments': 2, 'rank': 1}, {
    ...
    def test_get_top(self):
        ...
        response = self.client.get(
            reverse('top'), {
                'start_date': self.start_date, 'end_date': self.end_date_short})
