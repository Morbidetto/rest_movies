...
def get_movie_info(title):
    ...
    try:
        ...
        return Response({"Error": "Can't connect to external API"}, status=status.HTTP_503_SERVICE_UNAVAILABLE)
    data = r.json()
    if 'Type' in data and data['Type'] != 'movie':
            return Response({"Error": "Requested content isn't a movie"}, status=status.HTTP_400_BAD_REQUEST)
